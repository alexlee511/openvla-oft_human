...
class MultiThreadedDatasetBuilder(tfds.core.GeneratorBasedBuilder):
    ...
    def _download_and_prepare(self, dl_manager: download.DownloadManager, download_config: download.DownloadConfig) -> None:
        ...
        split_info_futures = []
        for split_name, generator in utils.tqdm(split_generators.items(), desc="Generating splits...", unit=" splits", leave=False):
...
def parse_examples_from_generator(paths, fcn, split_name, total_num_examples, features, serializer):
    generator = fcn(paths)
    outputs = []
    for sample in utils.tqdm(generator, desc=f"Generating {split_name} examples...", unit=" examples", total=total_num_examples, leave=False, mininterval=1.0):
        if sample is None:
            continue
        key, example = sample
        try:
...
class ParallelSplitBuilder(split_builder_lib.SplitBuilder):
    ...
    def _build_from_generator(self, split_name: str, generator: Iterable[KeyExample], filename_template: naming.ShardedFileTemplate, disable_shuffling: bool) -> _SplitInfoFuture:
...
def chunks(items, n):
    ...
    for i in range(n):
        start = (d + 1) * (i if i < r else r) + d * (0 if i < r else i - r)
        yield items[start:start + (d + 1 if i < r else d)]


def chunk_max(items, n, max_chunk_sum):
    out = []
    while items:
        out.append(list(chunks(items[:max_chunk_sum], n)))
        items = items[max_chunk_sum:]
    return out